...
# 1. checks tests between folds do not overlap
# 2. checks train, test, val does not overlap within a fold
# 3. checks cv, heldout test, ensemble data do not overlap
# idx_dictionary:
# 'cross_val' -> fold_idx -> 'test_ind'/'train_ind'/'val_ind' -> indices
# 'heldout_test_ind' -> indices
# 'ensemble_ind' -> indices
...
# all_data: tweet_id -> record ('word_int_arr', 'char_int_arr', 'user_post', 'user_mentions', 'user_retweet',
# 'created_at'
def retrieve_content(labeled_corpuses, unlabeled_corpuses, verbose):
    all_data = {}
    for corpus_dir in labeled_corpuses + unlabeled_corpuses:
        #corpus_dir records have attribute "tweet_id", "user_name", "text"
        if verbose:
...
def user_time_indexing(data_dictionary):
    ...
    for tweet_id in data_dictionary:
        ...
        if user_post == 1: #user is UNKNOWN
            continue
        else:
    ...
    return indexing #indexing: user_id -> list of tweet_ids sorted in time order from earliest to latest
...
def create_dataset(labeled_corpuses, unlabeled_corpuses, verbose=False):
    ...
    #'data'->data_dictionary: tweet_id -> record ('word_int_arr', 'char_int_arr', 'user_post', 'user_mentions',
    #                               'user_mentions', 'user_retweet', 'created_at', 'label' for labeled tweets)
    #'classification_ind'->idx_dictionary: 'cross_validation'->fold->'train_ind'/'test_ind'/'val_ind'
    #                'heldout_test_ind', 'ensemble_ind'
    #'user_time_ind'->user_time_ind: user_id -> list of tweet_ids sorted in time order from earliest to latest
    #'unlabeled_tr'->unlabeled_tr: list of unlabeled train tweet_ids
    #'unlabeled_val'->unlabeled_val: list of unlabeled validation tweet_ids
    ...
    #'data'->data_dictionary: tweet_id -> labeled record ('word_int_arr', 'char_int_arr', 'user_post', 'user_mentions',
    #                               'user_mentions', 'user_retweet', 'created_at', 'label' for labeled tweets)
    #'classification_ind'->idx_dictionary: 'cross_validation'->fold->'train_ind'/'test_ind'/'val_ind'
    #                'heldout_test_ind', 'ensemble_ind'
