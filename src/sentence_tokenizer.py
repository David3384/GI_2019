...
def unicode_rep(arr, option='word'):
    if option == 'char':
        ...
        return ' '.join([id2word[id].decode() for id in arr])
    else:
    ...
    if option == 'word':
        for token in utf8encode.split(b' '):
            if token in word2property and word2property[token]['id'] < vocab_count:
                ...
                result.append(1) #idx 1 is _UNKNOWN_
        if debug:
            print('mapping back to unicode string: %s' % ' '.join([id2word[id].decode() for id in result]))
        return result
    elif option == 'char':
        print(utf8encode)
