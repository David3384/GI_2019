...
tr_val_test_split = [0.64, 0.16, 0.2]

# split the data set into proportions
def split_by_proportion(tids, proportions):
    num_tweets = len(tids)
    # every proportion has this many tweets
    count_per_list = [int(p * num_tweets) for p in proportions]
    results, proportion_starts_at = [], 0

    # each proportion starts from <start>, and consider the next <count> elements
    for count in count_per_list:
        proportion_ends_at = proportion_starts_at + count
        results.append(tids[proportion_starts_at: proportion_ends_at])
        proportion_starts_at = proportion_ends_at
    return results
...
class Data_loader:
    ...
    def __init__(self, vocab_size = 40000, max_len = 50,
        ...
        load_token_option = 'word' if option == 'word' or option == 'both' else 'char'
        self.token2property = pkl.load(open('../model/' + load_token_option + '.pkl', 'rb'))  # loading the preprocessed token file
        removed_tokens = []
        ...
        self.separator = ' ' if option == 'word' or option == 'both' else ''  # chars are not seperated, words are by spaces
        if option == 'word' or option == 'both':  # creating an id2wtoken dictionary
    ...
    # get the training set, validation and test set by time
    # group by label: if true split proportionally within each label class
    def data_by_time(self, group_by_label):
        
        if not group_by_label:
            tr_ind, val_ind, test_ind = split_by_proportion(self.data['cv_by_time'],
                                                            tr_val_test_split)
        else:
            all_labels = self.data['cv_label_by_time'].keys()
            tr_ind, val_ind, test_ind = [], [], []
            for label in all_labels:
                # for each label, split data by fold
                tr, va, te = split_by_proportion(self.data['cv_label_by_time'][label],
                                                 tr_val_test_split)
                # add each split to the corresponding data set
                tr_ind += tr
                val_ind += va
                test_ind += te
        
        # get the train, val and test set
        tr, val, test = (self.get_records_by_idxes(tr_ind),
                         self.get_records_by_idxes(val_ind),
                         self.get_records_by_idxes(test_ind))
        return tr, val, test
...
if __name__ == '__main__':
    ...
    elif debug_option == 'by_time':
        dl = Data_loader(vocab_size = 40000, max_len = 150)
        tr, val, test = dl.data_by_time(group_by_label=True)
        for key, dataset in zip(['tr', 'val', 'test'], dl.data_by_time(group_by_label=True)):
            print('printing statistics for %s set' % key)
            label_count = collections.defaultdict(int)
            for tweet_dict in dataset:
                label_count[tweet_dict['label']] += 1
            print(label_count)
            print('start from: ', end='')
            print(min([tweet_dict['created_at'] for tweet_dict in dataset]), end=', ')
            print('ends at ', end='')
            print(max([tweet_dict['created_at'] for tweet_dict in dataset]), end='.')
            print()
