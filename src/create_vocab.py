...
def count_occurrence(corpus_dirs):
    corpus_word_count, corpus_char_count, user_post_count, user_rt_count, user_mentioned_count = \
        [{} for _ in range(5)]
    #corpus_word_count: word_byte_string -> count in corpus
    #corpus_char_count: char -> count in corpus
    #user_post_count: user_name -> count in corpus
    #user_rt_count: user_name -> number of retweets
    #user_mentioned_count: user_name -> number of users
    tweet_id_read = set()
...
def add_dictionary(dict_list): #used to merge counts for different corpus directories
    result = {}
...
def get_token_properties(labeled_corpus_token_count, unlabeled_corpus_token_count, user_name=False):
    ...
    # only consider tokens that occur more than once in labeled corpus + unlabeled corpus
    ...
    #token2property:
    #word -> 'occurrence_in_labeled' -> count
    #word->'occurrence in unlabeled'-> count
    #word->'isemoji' -> boolean
    #word->'id'->idx (sorted in order of frequency)
...
def create_vocab(labeled_corpuses, unlabeled_corpuses, word_file_dir, char_file_dir, user_file_dir, verbose=False):
    ...
    #user2property:
    #user -> 'occurrence_in_labeled' -> count
    #user ->'occurrence in unlabeled'-> count
    ...
    #user -> 'occurrence_in_labeled' -> count
    #user ->'occurrence in unlabeled'-> count
    #user -> 'labeled_user_post' -> count
    #user -> 'labeled_user_rt' -> count
    #user -> 'labeled_user_mentioned' -> count
    #user -> 'unlabeled_user_rt' -> count
    #user -> 'unlabeled_user_mentioned' -> count
    #user -> 'id' -> idx (sorted in order of frequency)
